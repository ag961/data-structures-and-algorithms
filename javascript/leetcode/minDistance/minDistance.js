'use strict';

/* Given two strings word1 and word2, return the minimum number of steps required to make word1 and word2 the same.

In one step, you can delete exactly one character in either string.

Example 1:

Input: word1 = "sea", word2 = "eat"
Output: 2
Explanation: You need one step to make "sea" to "ea" and another step to make "eat" to "ea".
Example 2:

Input: word1 = "leetcode", word2 = "etco"
Output: 4

Constraints:

1 <= word1.length, word2.length <= 500
word1 and word2 consist of only lowercase English letters. */

let minDistance = function (word1, word2) {
  let hashmap1 = new Map();
  let shorterWord = word1.length < word2.length ? word1 : word2;
  let longerWord = word1.length >= word2.length ? word1 : word2;

  let steps = 0;

  for (let i = 0; i < shorterWord.length; i++) {
    hashmap1.set(shorterWord[i], i)
  }

  for (let i = 0; i < longerWord.length; i++) {
    if (!hashmap1.has(longerWord[i])) {
      steps = steps + 1;
    } else {
      hashmap1.delete(longerWord[i])
    }
  }

  if (hashmap1.size > 0){
    steps = steps + hashmap1.size
  }

  return steps;
}

console.log(minDistance('sea', 'eat'));
console.log(minDistance('sea', 'ate'));
console.log(minDistance('leetcode', 'etco'));

module.exports = minDistance;
