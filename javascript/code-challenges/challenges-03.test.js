...
const addTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    let valuePlusTwo = arr[i] + 2;
    newArray.push(valuePlusTwo);
  }
  return newArray;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let newArray = arr.filter(number => !forbiddenValues.includes(number));
  return newArray;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let newArray = arr.filter(obj => obj.baseStat > minBaseStat);
  return newArray;
};
...
const getStatName = (arr, minBaseStat) => {
  let entireObjArr = getBaseStatGreaterThan(arr, minBaseStat);
  let nameOnlyArray = entireObjArr.map(obj => obj.stat.name);
  return nameOnlyArray;
};
...
const getCharactersWithoutChildren = (arr) => {
  let withouChildren = arr.filter(characterObj => !characterObj.children);
  return withouChildren;
};
...
const evenOddNumericValues = (arr) => {
  let numbersOnly = arr.filter(value => typeof value === 'number');
  let evenOdd = numbersOnly.map(num => {
    if (num % 2){
      return 'odd';
    } else {
      return 'even';
