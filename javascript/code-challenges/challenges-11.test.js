...
const totalSum = (input) => {
  return input.map(arr => arr.reduce((accumulator, currentvalue) => accumulator += currentvalue))
...
let findMaleAndFemale = (data) => {
  return data.filter(character => character.gender === 'female' || character.gender === 'male')
...
let findShortest = (data) => {
  let shortestHeight = data.reduce((acc, data)=> acc < Number(data.height) ? acc : Number(data.height));
  return data.filter(character => Number(character.height) === shortestHeight)[0].name;
};
