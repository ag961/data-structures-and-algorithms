...
const raisedToTheThird = (arr) => {
  let newArray = [];
  arr.forEach(num => {
    let numInPowerThree = Math.pow(num, 3);
    newArray.push(numInPowerThree);
  });
  return newArray;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    let raisedNumber = Math.pow(2, arr[i]);
    newArray.push(raisedNumber);
  }
  return newArray;
};
...
const forEachTwoToThe = (arr) => {
  let arrTwoToThe = [];
  arr.forEach(num => {
    let numRaised = Math.pow(2, num);
    arrTwoToThe.push(numRaised);
  });
  return arrTwoToThe;
};
...
const extractAbilities = (arr) => {
  let abilityNames = arr.map(objectWithProperties => objectWithProperties.ability.name);
  return abilityNames;
};
